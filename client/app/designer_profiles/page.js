  ...
  useEffect(() => {
    fetch('/api/designer_profiles')
      ...
      .then(data => {
        ...
        // Example soft filtering
        const filtered = data.filter(d => {
          const matchLocation = filters.location ? d.location.toLowerCase().includes(filters.location) : true
          const matchProjectType = filters.projectType ? d.bio?.toLowerCase().includes(filters.projectType) : true
          return matchLocation && matchProjectType
        })
    ...
    <div className={`min-h-screen bg-gradient-to-b 
      ...
      {filteredDesigners.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center mt-16 text-gray-500">
          ...
          <button
            onClick={() => setFilteredDesigners(designers)} // resets to full list
