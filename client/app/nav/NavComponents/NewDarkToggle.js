    ...
    const handleToggle = () => {
      ...
      return (
        <>
          ...
          {/* Sidebar */}
          <motion.div
            className={`fixed top-4 right-0 transform z-2`}
            initial={{ scale: 1, y: -100, opacity: 0}} // Start bigger and above the screen
            animate={{
              scale: isOpen ? 1 : .5, // Grow and shrink
              y: isOpen ? 0 : -10,
              x: isOpen ? 0 : 100,
              opacity: isOpen ? 2 : 0, // Translate down and up
            }}
            transition={{
              scale: { duration: 0.7},
              y: { duration: 0.7 },
          ...
          >
            <button
              ...
              className={`absolute -top-2 left-1/2 transform -translate-x-1/2 z-10 ${
                  theme === "light" ? "text-black" : "text-[#5FBB46]"}`} // Position on top
            >
              <Menu className="w-6 h-6" />
            </button>
            <div className="flex items-center justify-between gap-3 p-4 z-4 rounded-full">

              
              {/* Sun Icon */}
              <Sun
                className={`w-5 h-5 ${theme === "light" ? "text-black" : "text-[#5FBB46]"}`}
              />
    
              {/* Toggle Switch */}
              <button
                onClick={handleToggle}
                className={`relative w-10 h-10 rounded-full flex items-center pt-4 ${
